...
```shell
git clone --recurse-submodules https://github.com/riscv-non-isa/riscv-toolchain-conventions.git && cd riscv-toolchain-conventions && git submodule update --init --recursive
...
```shell
cd ./riscv-toolchain-conventions && make build
```
