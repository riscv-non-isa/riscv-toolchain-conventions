...
The compiler may accept `-march=native` to query the ISA from the host CPU.
This may not work for all CPUs or systems.
...
the `-mcpu` option, and it may result in an error if no `-mcpu` option
...
== Specifying the target CPU with -mtune and -mcpu

The compiler accepts the `-mtune` and `-mcpu` flags to specify the target CPU.
The `-mtune` options tells the compiler to optimize code for the specified CPU's
microarchitecture. It does not affect what ISA is used.

The `-mcpu` option selects the ISA of the specificed CPU if no `-march` is
provided. If no `-mtune` is provided, it also selects CPU microarchitecture to
optimize for.

The supported CPU names are implementation dependent.

The compiler may accept `-mtune=native` and `-mcpu=native` to ask the compiler
to target the host CPU. This may not work for all CPUs or systems.
...
Newer assemblers have `.option exact` which prevents compression and relaxation
without changing the enabled architecture. This ensures the toolchain will not
change the written sequence.
