...
model for the `sifive-x280` CPU that assumes DLEN (data path width in bits) being
half of VLEN, `-mtune=sifive-x280:no-dlen-factor-2` delivers the same set of
performance characteristics as `sifive-x280` except it no longer assumes DLEN being
half of VLEN. In other words, `no-dlen-factor-2` is a directive that _disable_
tuning feature. Directives of this kind are also know as *negative directive*.
A **positive directive**, on the other hand, enables an additional tuning feature in
the associated tuning model. For example, `-mtune=sifive-x280:single-element-vec-fp64`
represents a set of performance characteristics that is a union of that from `sifive-x280`
and `single-element-vec-fp64`, which assumes certain FP64 vector operations to
produce a single element per cycle. The `single-element-vec-fp64` in this case
is a positive directive.
...
reasons, and the compiler may optionally emit a warning.
...
features that are implied _implicitly_. For example, `short-forward-branch-opt`
and `no-short-forward-branch-opt` are the positive and negative directives for
feature `short-forward-branch-opt`, respectively. The presence of _feature_
`short-forward-branch-opt` implies the presence of two other tuning features,
`short-forward-branch-i-mul` and `short-forward-branch-i-minmax`.
Both of these two implied features follow a similar naming convention for their
directives: for example, `no-short-forward-branch-i-minmax` is the negative
directive for feature `short-forward-branch-i-minmax`.
Given this context, we have the following examples:

`-mtune=rocket:short-forward-branch-opt` is a valid string, obviously. This results
in `short-forward-branch-opt`, `short-forward-branch-i-mul`, and `short-forward-branch-i-minmax`
being _enabled_ among `rocket` 's existing performance characteristics.

`-mtune=rocket:no-short-forward-branch-i-mul` is a valiad string. This results in
both `short-forward-branch-i-mul` and `short-forward-branch-opt` being _disabled_
from `rocket` 's performance characteristics.

`-mtune=rocket:short-forward-branch-opt,no-short-forward-branch-opt` is *invalid*.
We do not allow these directives to "cancel out" each other.

`-mtune=rocket:short-forward-branch-opt,short-forward-branch-opt` is *invalid*.
We do not allow more than one occurrence of the same directive.

`-mtune=rocket:short-forward-branch-opt,no-short-forward-branch-i-mul` is *invalid*.
The `short-forward-branch-i-mul` produced by `short-forward-branch-opt` cannot
be canceled out by `no-short-forward-branch-i-mul`.

This _non-overlapping_ rule is a deliberated decision to simplify the logics and prevent
this from becoming unmanageable in the future.

It is worth noting that the actual tuning feature name is an implementation detail -- it
can be completely different from its directive names. For example, a tuning feature
`foo` can be dictated by directives `xyz` and `no-xyz`.

=== List of common tuning features
Here is a list of directives for tuning features that are currently shared by major compilers.
Each entry starts with `<positive directive name> / <negative directive name>`, followed
by description of the performance characteristics enabled by the positive directive.

- `short-forward-branch-opt` / `no-short-forward-branch-opt`: Has short forward branch
optimization, which effectively predicates instructions of certain types that follows
immediate after a forward branch.
- `optimized-zero-stride-load` / `no-optimized-zero-stride-load`: Vector strided loads
with a zero stride performs fewer memory operations.
