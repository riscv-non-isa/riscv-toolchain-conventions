...
The save restore optimization is enabled through the option `-msave-restore` and
reduces the amount of code in the prologue and epilogue by using library
sequences instead of inline code to save and restore callee saved registers.
These sequences are not functions as they do not follow the regular calling
convention.

The library sequences are provided in the emulation library and have the
following names:

- `+__riscv_save_<N>+`
- `+__riscv_save_cfiss_<N>+`
- `+__riscv_save_swss_<N>+`
- `+__riscv_restore_<N>+`
- `+__riscv_restore_cfiss_<N>+`
- `+__riscv_restore_swss_<N>+`
- `+__riscv_restore_tailcall_<N>+` (LLVM/compiler-rt only)
- `+__riscv_restore_tailcall_cfiss_<N>+` (LLVM/compiler-rt only)
- `+__riscv_restore_tailcall_swss_<N>+` (LLVM/compiler-rt only)

`<N>` is a value between 0 and 12 and corresponds to the number of registers
between `s0` and `s11` that are saved/restored. The return address register `ra`
is always included in the registers saved and restored. When using the `ILP32E`
ABI, `<N>` can be at most 2, as `s3` to `s12` are temporary registers in this
ABI.

The `+__riscv_save_*+` sequences are called from the prologue, using `t0` as the
link register (to avoid clobbering `ra`). They allocate stack space for the
registers and then save `ra` and the appropriate number of registers from
`s0`-`s11`.

The `+__riscv_restore_*+` sequences are tail-called from the epilogue. They
restore the saved registers, deallocate the stack space for the registers, and
then perform a return through the restored value of `ra`.

The `+__riscv_restore_tailcall_*+` sequences can be used used when the epilogue
of the called function ends in a tail-call. These restore the saved registers,
deallocate the stack space for the registers, and then tail-call to the address
of the tail-called function provided in register `t1`.

NOTE: The `+__riscv_restore_tailcall_*+` versions are not widely available in
toolchains as of late 2026.

The sequences with names that include `+_cfiss_+` must contain the `+sspush+` or
`+sspopchk+` instructions as used by the Zicfiss extension for hardware shadow
stack checking. These must be provided in objects that are correctly marked
(following the ABI) to denote their compatibility with the `Zicfiss` shadow
stack mechanism.

The sequences with names that include `+_swss_+` must save or restore the value
of `ra` using the software shadow stack pointed to by `x3`. The value of `ra`
may also need to be saved to the conventional stack to follow the frame pointer
convention. These must be provided in objects that are correctly marked
(following the ABI) to denote their use of `gp` for the software shadow stack.
