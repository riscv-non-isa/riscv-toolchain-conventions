...
== Specifying tuning processor and tuning features with -mtune

Compilers accept the `-mtune` option to specify the tuning processor model and
features. They allow users to compile with the assumption of certain performance
characteristics. Tuning processors and tuning features should not affect any of
the _architectural_ properties or behaviors.

The `-mtune` option accepts the following syntax: `-mtune=<tune-cpu>[":"<tune-features>]?`,
in which
[source]
----
...
feature within the tuning model for the named CPU. For example, given a tuning
model for the `sifive-x280` CPU that has short forward branch optimization,
`-mtune=sifive-x280:no-short-forward-branch-opt` delivers the same set of
performance characteristics as `sifive-x280` except it no longer assumes the
existence of short forward branch optimization. In other words, `no-short-forward-branch-opt`
is a directive that _disables_ tuning feature. Directives of this kind are also
known as *negative directive*. A **positive directive**, on the other hand, enables
an additional tuning feature in the associated tuning model. For example,
`-mtune=sifive-x280:single-element-vec-fp64` represents a set of performance
characteristics that is a union of that from `sifive-x280` and `single-element-vec-fp64`,
which assumes certain FP64 vector operations to produce a single element per cycle.
The `single-element-vec-fp64` in this case is a positive directive.

It is in our best interests to align the directives that are supported by multiple compilers.
However, it is understandable that the set of supported directives might vary from
one compiler to another. Therefore, unrecognized directives are ignored for compatibility
reasons, and the compiler is expected to emit a warning.

The tuning features enabled by positive directives in a tune feature string cannot
overlap with tuning features disabled by negative directives in the same string.
This mandate applies to explicitly induced tuning features as well as tuning
features that are implied _implicitly_. For example, `short-forward-branch-i-mul`
and `no-short-forward-branch-i-mul` are the positive and negative directives for
feature `short-forward-branch-i-mul` -- a specific kind of short forward branch
optimization, respectively. Since it represents a specific kind of short forward
branch optimization, the presence of this feature implies the presence of
another _feature_ `short-forward-branch-opt`, which represents the general short forward
branch optimization. This feature also follows a similar naming convention for its directives:
`short-forward-branch-opt` for positive and `no-short-forward-branch-opt` for negative.
Given this context, we have the following examples:

`-mtune=rocket:short-forward-branch-i-mul` is a valid string, obviously. This results
in both `short-forward-branch-i-mul` and `short-forward-branch-opt` being _enabled_
among `rocket` 's existing performance characteristics.

`-mtune=rocket:no-short-forward-branch-opt` is a valid string. This results in
both `short-forward-branch-opt` and `short-forward-branch-i-mul` being _disabled_
from `rocket` 's performance characteristics.

`-mtune=rocket:short-forward-branch-i-mul,no-short-forward-branch-i-mul` is *invalid*.
We do not allow these directives to "cancel out" each other.

`-mtune=rocket:short-forward-branch-i-mul,short-forward-branch-i-mul` is *invalid*.
We do not allow more than one occurrence of the same directive.

`-mtune=rocket:short-forward-branch-i-mul,no-short-forward-branch-opt` is *invalid*.
The `short-forward-branch-opt` implied by `short-forward-branch-i-mul` cannot
be canceled out by `no-short-forward-branch-opt`.

This _non-overlapping_ rule is a deliberate decision to simplify the logic and prevent
...
Zilsd is an extension for making 64-bit accesses on RV32. As with other
accesses, the spec only guarantees this will work for naturally-aligned accesses
(8-byte alignment). Some RV32 cores may have a memory system that relaxes this
requirement somewhat but not fully, allowing 4-byte aligned accesses but not
1-byte aligned accesses.

To enable this, there is the flag `-mzilsd-word-align` which allows Zilsd
accesses to be made with 4-byte alignment, or `-mzilsd-strict-align` which
requires Zilsd accesses to be made at their natural (8-byte) alignment.

These options interact with and may be modified by `-m[no-]strict-align` and
`-m[no-]scalar-strict-align`. Whichever is last on the command-line takes
precedence for Zilsd accesses.
