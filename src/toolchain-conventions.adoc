...
The profile name is expanded to its base ISA and mandatory extensions.

For a list of valid RISC-V profile names refer to the
https://github.com/riscv/riscv-isa-manual/blob/main/src/profiles/profiles.adoc[RISC-V profile specification],
...
This section lists common RISC-V specific toolchain driver command line options.

These would be passed to `clang` or `gcc` (or their C++ equivalents).
...
== Common Linker Command-Line Options

This section lists some common RISC-V specific linker command line options.

These would be passed directly to the linker, or via the toolchain driver using
`-Wl,<flag>`.

=== `--relax`/`--no-relax`

These enable or disable all RISC-V linker relaxation processing. Linker
relaxations are enabled by default.

=== `--relax-gp`/`--no-relax-gp`

These enable or disable the subset of RISC-V linker relaxations that assume that
`gp` (`x3`) contains the value of `__global_pointer$`, which are incompatible
with software shadow stack and other approaches.

This subset of relaxations are enabled by default, but are disabled if all
linker relaxations are disabled.

=== `--relax-tbljal`/`--relax-tbljal=[zcmt|*]`/`--no-relax-tbljal`

These enable or disable specific variants of the Table Jump linker relaxations.
These linker relaxations are disabled by default, and disabled if all linker
relaxations are disabled.

The `--relax-tbljal` option takes the name of a specific table jump scheme to
create a table for. If this is omitted, it is treated as being `zcmt`, but
vendors can use their own strings to denote vendor-specific variants of the
scheme.

NOTE: This option is required because the relaxation is not local: it produces a
single jump table used by all the relaxed instructions in the executable, so the
linker needs to know which kind of table to create if these are incompatible.

=== `-z zicfilp=[implicit|unlabeled|func-sig|never]`

Choose how the output image will be annotated for the Zicfilp scheme. By
default, the labelling is based on how all the input files are labelled
(`implicit`), however the label can be removed with `never` or forced to a
specific value otherwise.

NOTE: To understand why a particular annotation is being produced with
`implicit`, use the `-z zicfilp-*-report=` options below.

=== `-z zicfilp-unlabeled-report=[warning|error|none]`

Enable linker warnings/errors about which input objects lack the compatibility
annotations for the unlabelled Zicfilp scheme. These are disabled by default.

=== `-z zicfilp-func-sig-report=[warning|error|none]`

Enable linker warnings/errors about which input objects lack the compatibility
annotations for the function signatures Zicfilp scheme. These are disabled by
default.

=== `-z zicfiss=[implicit|always|never]`

Choose how the output image will be annotated for the Zicfiss scheme. By
default, the labelling is based on how all the input files are labelled
(`implicit`), however the label can be removed with `never` or added using
`always`.

NOTE: To understand why a particular annotation is being produced with
`implicit`, use the `-z zicfiss-report=` option, below.

=== `-z zicfiss-report=[warning|error|none]`

Enable linker warnings/errors about which input objects lack the compatibility
annotations for hardware shadow stack Zicfiss scheme. These are disabled by
default.
